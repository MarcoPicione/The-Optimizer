...
def main():
    ...
    file = "validation_problem_paretos"    
    ...
    plt.title('Validation problem', fontweight='bold', fontsize=fs + 2, pad = 10)
    ...
    plt.legend(handles, labels, prop={'weight':'bold', 'size': leg_fs}, scatterpoints=1, markerscale=2, fontsize=fs)

    plt.vlines(ref_point[0], ymin=-10, ymax=ref_point[1], color = 'black', linestyles="--", linewidth = lw)
    plt.hlines(ref_point[1], xmin=0, xmax=ref_point[0], color = 'black', linestyles="--", linewidth = lw)
    plt.savefig("dense_validation_problem.png")
